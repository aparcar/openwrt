...
class NinjaGenerator:
    ...
    def _generate_rules(self) -> list:
        ...
        return [
            ...
            '# Build toolchain',
            'rule toolchain',
            '  command = PYTHONPATH=$poc_dir BUILD_DIR=$build_root $python -m owrt -j $$(nproc) toolchain build $target && echo $keyhash > $builddir/stamp/toolchain.key && touch $out',
            '  description = Building toolchain for $target',
            '  pool = console',
            '',
            '# Build kernel',
            'rule kernel',
            '  command = PYTHONPATH=$poc_dir BUILD_DIR=$build_root $python -m owrt -j $$(nproc) kernel build $target && echo $keyhash > $builddir/stamp/kernel.key && touch $out',
